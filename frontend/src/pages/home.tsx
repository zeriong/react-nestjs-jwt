...
import {existToken, getCookie} from "../utile/authSetting";
import {useDispatch, useSelector} from "react-redux";
import {RootState} from "../store";
import {SET_TOKEN} from "../store/slices/auth";
import {SET_USER} from "../store/slices/userInfo";
        ...
        <>
            <div className="bg-sky-100 w-full h-full flex-col justify-center text-center py-8 relative">
                ...
                >
                    ...
                    <div className="flex flex-col">
                        <div className='text-2xl font-extrabold text-gray-600'>{(tokenState === null || false) ? (`로그인`) : (`${userState.name}님 ㅎㅇ`)}</div>
                        <Link to={'/userDetail'} className="border-8 border-sky-300 bg-sky-200 mt-3" >마이페이지</Link>
                        <button type="button" className="border-8 border-sky-300 bg-sky-200 mt-3">로그아웃(테스트)</button>
                    </div>
                </div>
            </div>
            <SignupModal/>
